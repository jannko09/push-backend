...
import * as pushes from "../routes/pushes";
import * as pushId from "../routes/pushes/{pushId}";

/* These must be the same as in the openapi.yaml */
export const routes: { [path: string]: typeof Route } = {
  "/pushes": pushes.ApiRoute,
  "/push/{id}": pushId.ApiRoute,
};
...
export class LambdaOpenApi {
  ...
  private routeResourceRewrite(routeInput: RouteInput) {
    ...
    for (let route in routes) {
      ...
      if (result.matched) {
        if (matchedRoute)
          /* If has been set before, error, just being extra safe, shouldn't happen */
    ...
    if (!matchedRoute)
      throw new HandledError(
        routeInput.method + " " + path + " does not match any routes"
      );
    ...
    if (
      routeInput.body &&
      !routeInput.isBase64Encoded &&
      !["get", "delete"].includes(routeInput.method)
    ) {
      try {
        routeInput.body = JSON.parse(routeInput.body);
      } catch (e) {
        routeInput.body = null;
      }
  ...
  private getRouteInputV1(event: APIGatewayProxyEvent) {
    ...
    if (!reqMethod) throw new Error("Request method is unknown");
    ...
    let routeInput: RouteInput = {
      ...
      params:
        (event.pathParameters as { [key: string]: string | string[] }) || {},
      query:
        (event.queryStringParameters as { [key: string]: string | string[] }) ||
        {},
      body: event.body,
  ...
  routeInput(event: APIGatewayProxyEvent | APIGatewayProxyEventV2): RouteInput {
    let routeInput = this.isAPIGWVersion2(event)
      ? this.getRouteInputV2(event)
      : this.getRouteInputV1(event);
  ...
  response(
    ...
    if (headers !== null) ret.headers = headers;
